'''
From https://github.com/nate-parrott/commanding.
Under MIT License.
'''
...
class ProbabilityCounter(object):
	...
	def items(self):
		return ((item, count * 1.0 / self.total) for (item, count) in self.counts.items())
...
def parse_phrase(text, examples, state_regexes=None):
	...
	for ex in examples:
		...
		states = list(map(lambda token_state: token_state[1], ex.token_state_tuples()))
	...
	for intent in intents:
		...
		for token in tokens + [None]:
			...
			for (candidate_log_prob, candidate_intent, candidate_states) in candidates:
				...
				for next_state_root, transition_prob in transition_probs[state].items():
					new_candidate = None
					if token == None:
